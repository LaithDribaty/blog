...
export const runApp = (app, scene, renderer, camera, enableAnimation = false, uniforms = getDefaultUniforms(), composer = null) => {
    ...
    const container = document.getElementById("heros-gallery-scene")
    ...
    window.addEventListener("resize", () => {
        ...
        renderer.setSize(container.clientWidth, container.parentElement.clientHeight)
    ...
    app.initScene()
        ...
        .then(() => {
            ...
            container.children[0].addEventListener('click', (e) => {app.lookAt(-1)})
            container.children[1].addEventListener('click', (e) => {app.lookAt(+1)})
        })
        .catch((error) => {
            console.log(error);
        });
